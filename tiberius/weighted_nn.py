...
PICLKLE_FILE_PATH = "data/train/weighted-nn-model.h5"
SUBMISSION_FILE_PATH = "data/test/weighted-nn-submission.csv"
...
model1 = Sequential()
model1.add(Embedding(len(word_index) + 1, 300,  weights=[embedding_matrix], input_length=100, dropout=0.2))
model1.add(LSTM(300, dropout=0.2, recurrent_dropout=0.2))

model2 = Sequential()
model2.add(Embedding(len(word_index) + 1, 300,  weights=[embedding_matrix], input_length=100, dropout=0.2))
