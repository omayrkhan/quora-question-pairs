...
#------------------------------------------------------------------------------------------------------------------------------------#
def words_to_vectors(fs1,contexsize,hs,negative,sample,file):
    ...
    context_size = contexsize  # window size to look number of words in context of the given word
    downsampling = sample  # Any number between 0 and 1e-5 is good for this (doesnot look repeatedly at the same words)
    ...
    negative = negative #if > 0, negative sampling will be used, the int for negative specifies how many "noise words"
    ...
    Write_to_csv(fs1, Features,num_features,file)
...
def Write_to_csv(fs1, Features,num_feaures,file):
    ...
    fs1.to_csv(file, columns=Features)
...
def Main():
    ...
    count=0
    for contexsize in range(2,6,3):
        for hs in range(0,2,1):
            for negative in range(5,11,5):
                for sample in range(1,5):
                    count+=1
                    file='Variant'+str(count)+".csv"
                    words_to_vectors(filtered_data, contexsize, hs, negative, 0.0000025 * sample, file)
